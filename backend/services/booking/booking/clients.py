...
class AccountsClient:
    """Is there a real person behind an owner? If so their requests wait for
    them; if not, the demo answers on their behalf."""

    async def has_account(self, owner_id: str) -> bool:
        raise NotImplementedError
...
class HttpAccountsClient(AccountsClient):
    def __init__(self, base_url: str) -> None:
        self._client = ServiceClient(base_url)
    ...
    async def has_account(self, owner_id: str) -> bool:
        ...
        try:
            await self._client.get(f"/internal/accounts/{owner_id}")
            return True
